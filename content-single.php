...
?>
	...
	<link itemprop="mainEntityOfPage" href="<?php the_permalink(); ?>" />
		...
		<?php if( get_post_meta( get_the_ID(), 'scope_featured_image_caption_text', true) != '' && has_post_thumbnail() ) {
			echo '<div class="scope-featured-image-caption clearfix">'. wp_kses_post( get_post_meta( get_the_ID(), 'scope_featured_image_caption_text', true ) ) .'</div>';
		} ?>
		...
		<?php if( get_post_meta( get_the_ID(), 'top_post_advertisement', true) != '' ) {
			echo '<div class="article-top-advertisement">'. do_shortcode( '[scope_ads id="'. esc_html(get_post_meta( get_the_ID(), 'top_post_advertisement', true)) .'"]' ) . '</div>';
		} ?>	
		...
		<div itemprop="articleBody" class="entry-content">
			...
			the_content();
			wp_link_pages( array(
				'before'      => '<nav class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'scope' ) . '</span>',
				'after'       => '</nav>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
			) );
		...
		<?php if( get_post_meta( get_the_ID(), 'bottom_post_advertisement', true) != '' ) {
			echo '<div class="article-bottom-advertisement">'. do_shortcode( '[scope_ads id="'. esc_html(get_post_meta( get_the_ID(), 'bottom_post_advertisement', true)) .'"]' ) . '</div>';
		} ?>
		
		<?php get_sidebar('post-content-bottom'); ?>
		<?php scope_post_meta_footer(); ?>
		<?php scope_post_links(); ?>
	...
	<?php if ( get_the_author_meta( 'description' ) && ot_get_option('author_description') != 'off' ) : ?>
		...
		<?php echo get_avatar( get_the_author_meta( 'ID' ), 100); ?>
			<div class="fn">
				<?php echo '<a class="url" href="'. esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )) .'" title="'. esc_attr(sprintf( __( 'View all posts by %s', 'scope' ), get_the_author() )) .'" rel="author">'. get_the_author() .'</a>'; ?>
			</div><!-- .fn -->	
