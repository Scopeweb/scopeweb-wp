	...
	<?php 
		...
		if( ot_get_option( 'post_review_rating' ) != 'off' && get_post_meta( get_the_ID(), 'enable_review', true ) == 'on' ){	
				if ( get_post_meta( get_the_ID(), 'review_breakdown', true ) == 'off' ) {
					...
					$rating = '<div class="mp-rating-wrapper"><div class="mp-rating-stars"><span style="width:'. esc_attr(scope_review_sum() * 10 ).'%"></span></div></div>';
				}
		} else {
			$rating = '';
		}
		...
		<?php if( $post_style == '2' ) : // Style 2 ?>
			...
			<div class="post-content-wrapper">
				...
				<header class="post-entry-header">
					<h2 itemprop="headline" class="entry-title"><a itemprop="mainEntityOfPage" href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'View %s', 'scope' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
				</header><!-- .entry-header -->
				...
				<?php echo $rating; ?>
					...
					echo '<div itemprop="articleBody" class="entry-summary">';
						$more_link_text = esc_html__('Read more','scope');
