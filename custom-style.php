...
*/
	...
	(ot_get_option('menu_color') != '') ? $custom_css .= '#secondary-navigation-wrapper{background-color:rgba('. scope_hex2rgb( ot_get_option('menu_color') ) .',0.98);}' : '';
	(ot_get_option('menu_color') != '') ? $custom_css .= '.header-search .searchform-wrapper {background-color:rgba('. scope_hex2rgb( ot_get_option('menu_color') ) .',0.9);}' : '';
	...
	if( is_page() || is_single() || is_category() ){
		if( $custom_header != '' ) {
			...
			(get_post_meta( $custom_header,'menu_color', true ) != '') ? $custom_css .= '#secondary-navigation-wrapper{background-color:rgba('. scope_hex2rgb( esc_attr(get_post_meta( $custom_header,'menu_color', true) )) .',0.98);}' : '';
			(get_post_meta( $custom_header, 'menu_color', true) != '') ? $custom_css .= '.header-search .searchform-wrapper {background-color:rgba('. scope_hex2rgb( esc_attr(get_post_meta( $custom_header, 'menu_color', true) )) .',0.9);}' : '';
...
*/
	...
	wp_add_inline_style( 'scope_main', $custom_css );
}

add_action('wp_enqueue_scripts', 'scope_custom_css');